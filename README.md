...
I plan to make it have the option between being screenless where the screen is replaced with a solar panel. It will not have notifications or GPS (which uses lots if power) or anything not health/time related. A later version might have GPS but it would be off by default.
